  ...
  Good[][] goods1= new  Good[m+1][4];
  
  for (int i = 1; i <= m; i++) { 
	  int v = scanner.nextInt(); 
	  int p =  scanner.nextInt(); 
	  int q = scanner.nextInt();//主件编号
  ...
  for (int i = 1; i <= m; i++) { 
    ...
    { //以下代码从分组中选择价值最大的。共五种情况：不选主件，选主件，选附件1和主件，选附件2和主件，选附件1和附件2和主件
      ...
      if (goods1[i][2] != null) { 
    	  if (j >= (vt = master.v + goods1[i][2].v) && max <  f[j - vt] + master.vp + goods1[i][2].vp) {
    		  max = f[j - vt] + master.vp +  goods1[i][2].vp; 
    	  } 
    	  if (j >= (vt = master.v + goods1[i][1].v + goods1[i][2].v) && max < f[j - vt] + master.vp +  goods1[i][1].vp + goods1[i][2].vp) { 
